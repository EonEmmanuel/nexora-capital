export function safeCapacityUtilization(allocated: number, capacity: number) {
  if (!Number.isFinite(allocated) || !Number.isFinite(capacity) || capacity <= 0) return 0;
...
export function enumParam<T extends string>(value: string | undefined, allowed: readonly T[]) {
