...
export async function getDashboardOverview(userId: string) {
  ...
  const totalAllocated = active.reduce(
    (sum, allocation) => sum.plus(allocation.initialValue),
    new Prisma.Decimal(0),
  );
  const currentValue = active.reduce(
    (sum, allocation) => sum.plus(allocation.currentValue),
    new Prisma.Decimal(0),
  );
  const totalReturn = currentValue.minus(totalAllocated);
  const distribution = active.map((allocation) => ({
    name: allocation.pool.name,
    value: allocation.currentValue,
  }));
  const deposits = transactions.filter(
    (transaction) => transaction.type === TransactionType.ALLOCATION,
  );
  return {
    allocations,
    active,
