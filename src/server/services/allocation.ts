...
export async function createAllocationIntent(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (user.status !== "ACTIVE")
      throw new Error("Account is not eligible for allocations.");
    const pool = await tx.tradingPool.findUniqueOrThrow({
      where: { slug: input.poolSlug },
    });
    const check = canAllocateToPool({
      amount,
      minimum: pool.minimumAllocation,
      maximum: pool.maximumAllocation,
      capacity: pool.totalCapacity,
      allocated: pool.currentlyAllocated,
      status: pool.status,
    });
    ...
    const allocation = await tx.allocation.create({
      data: {
        ...
        status: AllocationStatus.PAYMENT_PENDING,
        initialValue: amount,
        currentValue: amount,
      },
    });
    ...
    await tx.notification.create({
      data: {
        ...
        message: `Payment instructions are ready for ${pool.name}.`,
        type: "PAYMENT",
        actionUrl: `/payment/${payment.paymentReference}`,
      },
    });
...
export async function completeMockPayment(
  ...
  expectedCurrentStatus?: PaymentStatus | string,
) {
  assertMockPaymentControlsEnabled();
  return prisma.$transaction(async (tx) => {
    ...
    if (!payment.allocation)
      throw new Error("Payment is not attached to an allocation.");
    if (payment.status === PaymentStatus.COMPLETED) {
      if (expectedCurrentStatus && nextStatus !== "COMPLETED")
        throw new Error(
          "Payment cannot be advanced from its current status by the user flow.",
        );
      return payment;
    }
    if (nextStatus !== "COMPLETED") {
      if (expectedCurrentStatus) {
        const updated = await tx.payment.updateMany({
          where: {
            id: payment.id,
            status: expectedCurrentStatus as PaymentStatus,
          },
          data: {
            status: nextStatus,
            confirmations: nextStatus === "CONFIRMING" ? 1 : 0,
            receivedAmount: payment.requestedAmount,
          },
        });
        if (updated.count !== 1)
          throw new Error(
    ...
    const paymentClaim = await tx.payment.updateMany({
      where: {
        ...
        status: expectedCurrentStatus
          ? (expectedCurrentStatus as PaymentStatus)
    ...
    `;
    if (capacityUpdateCount !== 1)
      throw new Error("Payment would exceed remaining pool capacity.");
    await tx.allocation.update({
      where: { id: payment.allocationId! },
      data: {
        status: AllocationStatus.ACTIVE,
        activatedAt: new Date(),
        currentValue: payment.requestedAmount,
      },
    });
    await tx.transaction.upsert({
      ...
      create: {
        ...
        description: `Activated allocation ${payment.allocation.reference}`,
      },
    });
    await tx.notification.create({
      data: {
        ...
        message: `${payment.allocation.pool.name} is now active.`,
        type: "SUCCESS",
        actionUrl: `/dashboard/pools/${payment.allocationId}`,
      },
    });
    return tx.payment.findUniqueOrThrow({ where: { id: payment.id } });
  });
...
export async function completeUserMockPayment(
  userId: string,
  paymentReference: string,
  nextStatus: MockPaymentTransition,
) {
  ...
  const payment = await prisma.payment.findFirst({
    where: userOwnedPaymentWhere(userId, paymentReference),
    include: { allocation: true },
  });
  if (
    !payment ||
    payment.provider !== "mock" ||
    payment.allocation?.userId !== userId
  )
    throw new Error("Payment not found.");
  const expectedCurrentStatus = expectedUserMockPaymentSource(nextStatus);
  if (!canUserAdvanceMockPayment(payment.status, nextStatus))
    throw new Error(
      "Payment cannot be advanced from its current status by the user flow.",
    );
