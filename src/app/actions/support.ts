...
export async function replyTicketAction(ticketId:string,formData:FormData){const user=await requireUser();await prisma.supportMessage.create({data:{ticketId,authorId:user.id,body:String(formData.get('message')),internal:false}});redirect(`/dashboard/support/${ticketId}`)}
